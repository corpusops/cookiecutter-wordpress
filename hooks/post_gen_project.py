...
TEMPLATE = 'cookiecutter-wordpress'
SYMLINKS_DIRS = {
...
SYMLINKS_FILES = {
    ...
    ".dockerignore": "{{cookiecutter.deploy_project_dir}}/.dockerignore",  #noqa
...
git rm -rf \
   ...
   src/{{cookiecutter.wordpress_project_name}}/settings/instances/{{i}}* \
        || /bin/true
rm -rfv \
   .ansible/inventory/group_vars/{{i}} \
   src/{{cookiecutter.wordpress_project_name}}/settings/instances/{{i}}*
...
if [ -e Dockerfile ] && [ ! -h Dockerfile ];then
sed -i -re \
	"s/WORDPRESS_PHP_VER=.*/WORDPRESS_PHP_VER={{cookiecutter.wordpress_php_ver}}/g" \
	Dockerfile
sed -i -re \
	"s/project/{{cookiecutter.wordpress_project_name}}/g" \
	Dockerfile
fi
...
cp -fv {{cookiecutter.deploy_project_dir}}/local-env.php .
cp local/wordpress_project_name
while read f;do
