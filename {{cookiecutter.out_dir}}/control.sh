...
#       APP_USER=wordpress ./control.sh usershell ls /
...
#       APP_USER=wordpress ./control.sh userexec ls /
...
#       APP_USER=wordpress ./control.sh duserexec -> run interactive shell inside default CONTAINER
#       APP_USER=wordpress ./control.sh duserexec foo123 -> run interactive shell inside foo123 CONTAINER
#       APP_USER=wordpress ./control.sh duserexec wordpress_123 ls / -> run comand inside foo123 CONTAINER
do_duserexec() {
...
do_test() {
    ...
    set -- vv do_shell \
        ...
        && gosu www-data echo Wordpress tests noop called with $bargs"
    "$@"
}
