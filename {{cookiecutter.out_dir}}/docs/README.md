...
To add/modify wordpress settings, you specifically need to edit ``.ansible/playbooks/roles/wordpress_vars/defaults/main.yml`` for hosted deployments,<br/>
and adapt certainly one of ``cops_wordpress_docker_env_freeform``, ``cops_wordpress_docker_env_defaults``, ``cops_wordpress_docker_env_extra``.
  - Here, never forget to abuse ansible variables to cut variables from their templates<br/>
    ...
    eg:
      - In all cases, adapt/edit inventory (variables may already exist)
        ```sh
        $EDITOR .ansible/playbooks/roles/wordpress_vars/defaults/main.yml
        ```

          ```yaml
          # ...
          cops_wordpress_docker_env_extra:
            MYSETTINGS={{'{{'}}cops_wordpress_foobar}}
            MYSECRETSETTINGS={{'{{'}}cops_wordpress_footruc}}
          ```
      - Set variables values in related clear group vars
        ...
        ```
          ...
          ```yaml
          # ...
          cops_wordpress_foobar: supervalue
          ```
      ...
      - Set crypted values (see ansible/README about vaults) in related crypted group vars
        ...
        ```
          ...
          ```yaml
          # ...
          cops_wordpress_footruc: abcd123456789secret
          ```
